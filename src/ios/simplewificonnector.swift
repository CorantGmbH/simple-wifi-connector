...
@objc(simplewificonnector) class simplewificonnector : CDVPlugin {
  ...
  func connect(_ command: CDVInvokedUrlCommand) {
    ...
    var pluginResult = CDVPluginResult(status: CDVCommandStatus_ERROR, messageAs: "Network unavailable or wrong password")
    let configuration = NEHotspotConfiguration.init(ssid: ssid, passphrase: password, isWEP: false)
    configuration.joinOnce = true
    NEHotspotConfigurationManager.shared.apply(configuration) { (error) in
      if error != nil {
        if error?.localizedDescription == "already associated." {     
          pluginResult = CDVPluginResult(status: CDVCommandStatus_OK,messageAs: ssid)
        } else {
